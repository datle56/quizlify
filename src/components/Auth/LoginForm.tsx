...
const LoginForm: React.FC = () => {
    ...
    // Nếu dùng import { useLocation } from 'react-router-dom' ở đầu file:
    // const location = useLocation();
    // const state = location.state as { email?: string; password?: string };
    const [formData, setFormData] = useState({
        email: (state && state.email) || "demo@gmail.com",
        password: (state && state.password) || "demo123",
        rememberMe: false,
    })
    ...
    // Kiểm tra nếu đã authenticated thì redirect
    React.useEffect(() => {
    ...
    // Nếu nhận state có email/password thì tự động submit
    React.useEffect(() => {
        if (
            state &&
    ...
    }, [state, login, navigate, autoSubmitted])
